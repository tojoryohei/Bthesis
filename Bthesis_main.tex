...
\begin{figure}[H]
    ...
    \draw[path_arrow] (2, -1.2) -- node[below] {3,740円運賃計算キロ：219.6km　　} (12, -1.2);
...
第\ref{chapter:algorithm}章で後述する分割乗車券プログラムから呼び出されるプログラムであり，入力された経路を乗車することができる片道乗車券の中で最安となりうる経路全ての運賃計算を行い，最安となる経路と運賃の組み合わせを1つ出力する．

\section{運賃計算アルゴリズム}
運賃計算アルゴリズムでは，入力された経路に対して付録Aの規則による経路の補正を行い，以下のように規則に則った厳密な運賃を計算する．
\begin{itemize}
    \item
        並行する他社路線に対抗するために通常の距離制運賃よりも安く設定された区間である特定運賃区間と経路が一致する場合はその特定運賃を参照する．
    \item
    ...
    \item
        上記3つを運賃計算をした上で，鉄道バリアフリー料金該当区間相互発着の場合は鉄道バリアフリー料金を加えた額を計算して運賃に加える．\\
\end{itemize}
...
このうち，合計運賃が最安の片道乗車券の集合を，今回の研究では「最安分割乗車券」と定義する．
...
\section{実験概要}
    ...
    \begin{enumerate}
        \item \textbf{正当性:} 既存の分割計算サービスや実機（券売機）との一致確認．
        \item \textbf{有用性:} 分割による運賃削減効果の定量的評価．
        \item \textbf{性能:} 探索距離に応じた計算時間とボトルネックの特定．
\end{enumerate}
...
無作為に抽出した100区間において，駅の券売機および既存の運賃検索サービスと照合した結果，全区間で運賃が一致することを確認した．
特に，第\ref{chapter:fare_system}章で述べた「経路特定区間」や「特定都区市内」における外方延伸の逆転現象についても，本システムが自動的に最安経路を選択することを確認した．

\subsection{既存分割計算サービスとの比較}
    既存のWebサービス「oba氏の乗車券分割プログラム」を基準とし，同一経路における分割結果を比較した．
    表\ref{tab:comparison_oba}に，東京ー仙台間における比較結果を示す．
    ...
    \begin{table}[htbp]
        ...
        \begin{tabular}{lrrcc} \toprule
            ...
            oba氏のプログラム & 5,830円 & 最短経路 & 経路固定 \\
            \textbf{本システム} & \textbf{5,800円} & \textbf{迂回経路} & \textbf{経路自動探索}  \\ \bottomrule
        \end{tabular}
    \end{table}
...
第\ref{chapter:fare_system}章で分類した5つの分割要因に基づき，本システムによる削減効果を測定した．結果を表\ref{tab:effectiveness}に示す．
...
\begin{table}[htbp]
    ...
    \begin{tabular}{llrrr} \toprule
        ...
        特定運賃の利用 & 田端ー成田 & 1,170円 & 1,090円 & 80円 \\
        長距離逓減の利用 & 福山ー静岡 & 9,790円 & 9,700円 & 90円 \\
        都区市内の回避 & 宮島口ー網干 & 5,170円 & 4,600円 & 570円 \\
        電車特定区間 & 久宝寺ー大河原 & 990円 & 980円 & 10円 \\ \bottomrule
    \end{tabular}
\end{table}
...
\begin{table}[htbp]
    ...
    \begin{tabular}{lrr} \toprule
        ...
        近距離 ($d < 100$km) & X.XXs & X \\
        中距離 ($100 \le d < 500$km) & X.XXs & XX \\
        長距離 ($d300 \le d $km) & X.XXs & XXX \\ \bottomrule
    \end{tabular}
\end{table}

\section{考察}
実験の結果，本システムは全国規模の路線網において，経路指定なしで厳密な最安分割運賃を導出できた．

一方で，表\ref{tab:performance}に示す通り，探索距離が300kmを超える超長距離区間では，計算時間がXX秒を超過するケースが見られた．
これは，Yen's Algorithmによる第$K$最短経路の列挙において，ノード数に比例して候補経路が爆発的に増加するためである．
現状のアルゴリズムではWebサービス水準の即応性確保が困難となるリスクがあり ，今後は計算時間を短縮するための枝刈り条件のさらなる厳格化が必要である．

\section{結論}
\subsection{本研究の総括}
本研究では，JR分割乗車券の最適解を探索するプログラムをDijkstra's algorithmおよびYen's Algorithmを用いて開発した．
本システムにより，従来困難であった経路指定不要の広域探索が可能となり，複雑な運賃制度を厳密に考慮した上での最安解導出を実現した．
