...
動的計画法による厳密解の探索も検討したが，予備実験において探索空間の増大に伴いメモリ使用量が指数関数的に増加し，計算継続が困難となったため，計算コストを抑えつつ解の精度を維持できる本手法を採用した．
...
具体的には，抽出された経路候補数を $K$，経路内の駅数を $n$ としたとき，分割パターンの網羅的な探索に必要な計算量は $O(K \cdot 2^n)$ のオーダーとなる．
探索距離の増加に伴い候補経路数 $K$ と駅数 $n$ の双方が増加するため，全体の処理時間およびメモリ消費量が爆発的に増大することが原因である．
